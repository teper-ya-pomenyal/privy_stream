...
func (h *CatalogGRPCHandler) GetAlbumByID(ctx context.Context, req *catalogv1.GetAlbumByIDRequest) (*catalogv1.GetAlbumByIDResponse, error) {
	...
	respTracks := make([]*catalogv1.AlbumTrack, 0, len(album.Tracks))
	for _, t := range album.Tracks {
		respTracks = append(respTracks, &catalogv1.AlbumTrack{
	...
	return &catalogv1.GetAlbumByIDResponse{
		Album: &catalogv1.FullAlbum{
			AlbumUuid:  album.AlbumID.String(),
			AlbumName:  album.AlbumName,
			ArtistUuid: album.ArtistID.String(),
